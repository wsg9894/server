...
//                   "</tr>" +
    ...
    closeboxy: function(message) {
        jQuery('.boxy-wrapper').each(function(){
            if(jQuery(this).find('.loadCon').text() == message){
                jQuery(this).hide();
                jQuery(this).remove();
            }
        });
    ...
    codeAsk: function(answers, callback, options){
        ...
        var img_click="'";
        var body = jQuery('<div style="background: #fff;"></div>');
        var  string='<div class="security_box" style="display:block;" id="reg_yzm_code">';
               string+='<div class="security_row">';
                  string+='<div class="se_info"><input class="se_input" value="" id="code_value_yzm"/></div>';
                  string+='<div class="se_prefix">提示码</div>';
               ...
               string+='<div class="security_row">';
                   ...
                   string+='<a href="javascript:void(0)" class="safe_item_refresh" onclick="Boxy.codeOnclick();">看不清？</a>';
                string+='</div>';
                string+='<div class="security_row"><p style=" text-align: center; color: #666;">请输入提示码获取验证码</p></div>';
