...
class SiteController extends Controller
{
	...
	public function actionGoodsOrder()
	{
		...
		{
			GeneralFunc::alert('您还没有登录呦');
			GeneralFunc::gotoUrl('/usercenter/login.html?go='.Yii::app()->params['baseUrl'].'project/index.php?r=scoreStore/Site/goodsOrder&gid='.$gid.'&iUserSourceID=77');
			exit(0);
		}
		...
		Yii::app()->smarty->display('scoreStore/goodsOrder.html');
	}
	...
	public function actionAddAddress()
	{
		...
		if(!empty($_REQUEST['subFlag']) and $_REQUEST['subFlag']==1)
		{
			...
			{
				GeneralFunc::gotoUrl('/project/index.php?r=scoreStore/Site/goodsOrder&gid='.$gid);
				exit(0);
			}
	...
	public function actionGoodsExchange()
	{
		...
		if(!empty($_REQUEST['subFlag']) and $_REQUEST['subFlag']!=1)
		{
			GeneralFunc::gotoUrl('/project/index.php?r=scoreStore/Site/goodsOrder&gid='.$gid.'&aid='.$aid);
			exit(0);
		}
		...
		if(1!=$isExchangeRet)
		{
			...
			GeneralFunc::gotoUrl(Yii::app()->params['baseUrl'].'project/index.php?r=scoreStore/Site/goodsOrder&gid='.$gid);
			exit(0);
		}
		...
		$orderInfo = array('gid'=>$gid, 'goods_price'=>$goodsInfo['price'], 'goods_extra_price'=>$goodsInfo['extra_price']
				, 'goods_num'=>$payNum, 'goods_tprice'=>$goodsInfo['price']*$payNum, 'goods_extra_tprice'=>$goodsInfo['extra_price']*$payNum
				, 'order_amount'=>$goodsInfo['extra_price']*$payNum+$goodsInfo['send_cost'], 'order_name'=>$goodsInfo['name']
				, 'uid'=>$uSessInfo['iUserID'], 'user_addressid'=>$aid, 'order_pay_type'=>$payType
		);
...
// 			and (!empty($uSessInfo['activity']) and $uSessInfo['activity']=='score_store'.$uSessInfo['iUserID']))
	
		//lzz update at 2017-03-09，增加注册验证门槛（MD5(phone+当前日期+userid)）
		if($uid!=$uSessInfo['iUserID']
			and (!empty($uSessInfo['activity_verify']) and $uSessInfo['activity_verify']=='score_store'.md5($_SESSION["sPhone"].GeneralFunc::getCurDate().$_SESSION["iUserID"])))
		{
